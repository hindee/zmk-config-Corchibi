...
GitHubのトップページ(README.md)に表示されているキーマップ画像は、<br>
キーマップを編集するとGitHubのワークフローによって自動実行されます。<br>
Keymap-drawerを使ってキーマップ画像を描画する仕組みを導入しています。<br>


> [!NOTE]
>[キーマップエディタ](https://nickcoutsos.github.io/keymap-editor/)で編集してSaveするだけで自動実行されます。<br> 
>したがってキーマップ画像の色テーマ（※後述）に問題が無ければ、描画のためにすることは何もありません。
...
> [!NOTE]
> テーマを変えずにキーマップだけ更新する場合は、このファイルを触る必要はありません。

### [!TIP] お試しでテーマを一時的に変えたいとき

コミットせずに別テーマで描画を確認することが可能です。<br>
GitHubの **Actionsタブ** から手動で実行できます。
...
> [!NOTE]
> 手動実行の場合、`theme.txt` の内容は変更されません。<br>
> あくまでその1回の描画にのみ適用されます。
